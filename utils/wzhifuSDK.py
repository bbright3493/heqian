...
class WxPayConf_pub(object):
    ...
    NOTIFY_URL = "http://%s/wx/recharge/payback/" % settings.HOST

    #=======【JSAPI路径设置】===================================
    #获取access_token过程中的跳转uri，通过跳转将code传入jsapi支付页面
    JS_API_CALL_URL = "http://%s/pay/?showwxpaytitle=1" % settings.HOST
...
class UrllibClient(object):
    ...
    def postXml(self, xml, url, second=30):
        """
        不使用证书
        read得到的是string post需转换成bytes
        """

        data = urllib.request.urlopen(url, xml.encode('utf-8'), timeout=second).read()
        return data
